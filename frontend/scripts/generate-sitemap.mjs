 ...
 * - Protocol/flow routes from protocols/presentation/protocol-catalog.ts
 * 
 * Features:
 ...
 * 
 * Add protocols to protocol-catalog.ts and sitemap updates automatically.
 */
...
// =============================================================================
// Parse Protocol catalog metadata
// =============================================================================

function parseProtocols() {
  const protocolsPath = join(srcDir, 'protocols', 'presentation', 'protocol-catalog.ts');
  const content = readFileSync(protocolsPath, 'utf-8');
  
  const protocols = [];
  
  const protocolsArrayMatch = content.match(/export\s+const\s+PROTOCOL_CATALOG(?:\s*:[^=]+)?\s*=\s*\[([\s\S]*?)\n\]/);
  
  if (!protocolsArrayMatch) {
    console.warn('⚠️  Could not parse PROTOCOL_CATALOG from protocol-catalog.ts');
    return protocols;
  }
  ...
  const flowsRegex = /\{\s*id:\s*['"]([^'"]+)['"][\s\S]*?flows:\s*\[([\s\S]*?)\]\s*,?\s*\}/g;
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  <!-- 
    ...
    Routes parsed from: App.tsx, protocol-catalog.ts
...
function main() {
  ...
  try {
    ...
    console.log('   └─ src/protocols/presentation/protocol-catalog.ts\n');
