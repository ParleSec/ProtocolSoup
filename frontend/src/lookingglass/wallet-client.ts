...
const LOOKING_GLASS_FAILURE_MATCHERS: LookingGlassFailureMatcher[] = [
  ...
  },
  {
    ...
    guidance:
      'Choose a credential format that appears in the DCQL query (mso_mdoc or dc+sd-jwt), or paste a credential_jwt of that format. Looking Glass blocks one-click submit on a hard mismatch so the wallet is not asked to present the wrong evidence.',
