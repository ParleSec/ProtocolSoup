...
import type { WireCapturedExchange, WireCertificateInfo } from '../../types'
...
export function TLSInspector({ exchange }: TLSInspectorProps) {
  ...
  const bindingInfo = useMemo<TokenBindingInfo>(() => {
    ...
    const payload = token ? decodeJWTWithoutValidation(token)?.payload : undefined
  ...
  return (
    <div className="space-y-3">
      ...
      <InfoBlock title={`Context · ${sourceLabel}`}>
        <FieldRow label="Version" value={tls.version || 'unknown'} />
        ...
        <FieldRow label="mTLS" value={tls.mutualTLS === undefined ? 'unknown' : (tls.mutualTLS ? 'yes' : 'no')} />
        <FieldRow label="Chain Status" value={chainStatus} />
      </InfoBlock>

      <InfoBlock title="Client Certificate">
        {clientCert ? (
          <>
            <FieldRow label="Status" value={clientStatus.label} valueClassName={statusTone(clientStatus.tone)} />
            <FieldRow label="Subject" value={clientCert.subject} />
            <FieldRow label="Issuer" value={clientCert.issuer} />
            <FieldRow label="Serial Number" value={clientCert.serialNumber} mono />
            ...
            <FieldRow label="Thumbprint (SHA-256)" value={clientCert.thumbprint || 'unknown'} mono />
            {clientChainDisplay.length > 0 && <ChainList subjects={clientChainDisplay} />}
          </>
        ) : (
          <FieldRow label="Status" value="No client certificate presented" />
        )}
      </InfoBlock>
      ...
      <InfoBlock title="Server Certificate">
        {serverCert ? (
          <>
            <FieldRow label="Status" value={serverStatus.label} valueClassName={statusTone(serverStatus.tone)} />
            <FieldRow label="Subject" value={serverCert.subject} />
            <FieldRow label="Issuer" value={serverCert.issuer} />
            <FieldRow label="Serial Number" value={serverCert.serialNumber} mono />
            ...
            <FieldRow label="Thumbprint (SHA-256)" value={serverCert.thumbprint || 'unknown'} mono />
            {serverChainDisplay.length > 0 && <ChainList subjects={serverChainDisplay} />}
          </>
        ) : (
          <FieldRow label="Status" value={isOutbound ? 'Server certificate not available' : 'Not captured on inbound TLS'} />
        )}
      </InfoBlock>

      <InfoBlock title="Certificate Binding (RFC 8705)">
        <FieldRow label="Status" value={bindingStatus.label} valueClassName={bindingStatus.tone} />
        <FieldRow label="Client thumbprint" value={bindingInfo.clientThumbprint || 'unknown'} mono />
        <FieldRow label="Token cnf.x5t#S256" value={bindingInfo.tokenThumbprint || 'missing'} mono />
        {bindingInfo.token && (
          <FieldRow label="Authorization token" value={formatShort(bindingInfo.token, 60)} mono />
        )}
      </InfoBlock>
    </div>
  )
