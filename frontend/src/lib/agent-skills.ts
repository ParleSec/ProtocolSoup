...
\`Accept: text/markdown\` on \`${WALLET_ORIGIN}/\` returns the recipes without executing JavaScript.
...
- Session JSON: \`GET ${WALLET_ORIGIN}/api/session\` with \`X-Wallet-Session\`. For Looking Glass runs, also pass \`looking_glass_session_id\` in the JSON body.
...
This origin's \`/mcp\` server is read-only catalog/decode and does **not** proxy the wallet. Call ${WALLET_ORIGIN} directly.
`,
  },
]
