...
**Fix:** Provision reachable Redis with TLS, set its `rediss://` URL as a
platform secret, and verify network and certificate trust from the federation
service. Do not downgrade production to `redis://` or an in-memory store. A
runtime Redis error after startup fails the affected token or resource
request closed with `server_error`, rather than silently accepting an
unverified proof.
