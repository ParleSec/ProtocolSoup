...
| `OID4VP_X509_SANDNS_CLIENT_ID` | `(unset)` | Verifier `client_id` used when `client_id_scheme=x509_san_dns`. Must match a DNS SAN on the configured leaf certificate. |
| `OID4VP_X509_SANDNS_CERT_CHAIN_PEM` | `(unset)` | PEM-encoded certificate chain added to the request object's `x5c` JOSE header. |
| `OID4VP_X509_SANDNS_PRIVATE_KEY_PEM` | `(unset)` | PEM-encoded private key matching the leaf certificate used for `x509_san_dns` request signing. |
