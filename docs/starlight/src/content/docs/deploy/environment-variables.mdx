...
| `OIDC_KEY_ROTATION_TOKEN` | *(empty — disabled)* | Bearer token required for `POST /oidc/admin/rotate-keys`. Leave empty to keep the rotation endpoint disabled. Store as a platform secret when running the OIDF OP-rotation module. |
| `OIDC_CONFORMANCE_REDIRECT_URIS` | *(empty)* | Comma-separated static redirect URIs for pre-registered OIDF Basic/Implicit/Hybrid/Form-Post clients only. Do **not** add Dynamic OP or Third-Party Initiated Login callbacks here; those profiles register their callback URI dynamically. |
