...
| `CONFORMANCE_REPORT` | *(empty — verdicts hidden)* | Path to the JSON produced by `go run ./cmd/conformance-report` for this build. The gateway serves it through `/api/conformance/*`, which backs the `/spec/{spec}` and `/spec/{spec}/{requirement}` pages. A report is only used when its `commit` equals `BUILD_COMMIT` and its `dirty` flag is false; otherwise the pages render every requirement as *Not evaluated for this build* with `noindex`. CI generates the report before the Docker build and `Dockerfile.fly` sets this to `/app/artifacts/vc-conformance/report.json`. |
...
| `SSF_STREAM_CLIENT_SECRET` | *(auto-generated)* | Secret for the seeded `ssf-stream-client` confidential client (`client_credentials`, `ssf.read` / `ssf.manage`). Pin as a platform secret in production so Stream Management tokens stay usable across restarts. The current secret is listed at `GET /oauth2/demo/clients`. |
...
| `SSF_AS_INTROSPECT_URI` | - | RFC 7662 introspection URL used by the split SSF Transmitter to check Stream Management token revocation. |
