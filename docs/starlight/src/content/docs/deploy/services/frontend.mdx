...
Looking Glass also uses a same-origin `/wallet-harness/*` rewrite to the wallet harness (`WALLET_BACKEND_ORIGIN`).
...
| `WALLET_BACKEND_ORIGIN` | Yes (when Looking Glass uses a remote wallet) | `https://wallet.protocolsoup.com` | Wallet harness origin for `/wallet-harness/*` |
