...
- Looking Glass is the run surface: `/looking-glass?protocol=ssf`. The Flow tab is Looking Glass bus events only. Catalog flow IDs are presets into one durable stream lab session. Do not fold this image into federation.
