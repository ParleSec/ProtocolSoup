...
| `OID4VP_X509_SANDNS_CLIENT_ID` | No | - | `x509_san_dns` verifier identifier; must match a DNS SAN in the leaf certificate |
| `OID4VP_X509_SANDNS_CERT_CHAIN_PEM` | No | - | PEM-encoded certificate chain used for `x509_san_dns` request signing |
| `OID4VP_X509_SANDNS_PRIVATE_KEY_PEM` | No | - | PEM-encoded private key matching the configured leaf certificate |
...
- OID4VP verifier identities support `redirect_uri`, `did:web`, `verifier_attestation`, and `x509_san_dns` when the corresponding verifier identity material is configured.
