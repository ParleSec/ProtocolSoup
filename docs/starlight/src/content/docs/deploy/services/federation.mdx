...
- `GET|POST /oauth2/authorize` (`response_type=code`, or `token` for the implicit grant), `POST /oauth2/token`
- `POST /oauth2/device/authorize` (RFC 8628), `GET|POST /oauth2/device`
