...
| `SHOWCASE_DATA_DIR` | No | - | State directory for wallet credentials, verifier sessions, and the auto-provisioned OID4VP X.509 request signer |
...
- Set `OID4VP_X509_SANDNS_CERT_CHAIN_PEM` and `OID4VP_X509_SANDNS_PRIVATE_KEY_PEM` in production for stable x509_san_dns certificate continuity. Alternatively, set `SHOWCASE_DATA_DIR` to a durable volume: the generated signer is persisted at `{SHOWCASE_DATA_DIR}/oid4vp/`, and its CA root must be configured in every wallet through `WALLET_VERIFIER_X509_TRUST_ANCHOR_PEM`.
