...
- `POST /api/palette/query` -- content palette search (see [palette index](/deploy/palette-index/))
- `GET /api/.well-known/jwks.json` -- JSON Web Key Set

The start-demo response returns a 256-bit `session_token` owner capability
once. Session lists remain metadata-only. Full session GETs require
`X-Looking-Glass-Session-Token`; WebSocket subscriptions carry the capability
in `protocolsoup-lookingglass-owner.<session_token>` alongside the
`protocolsoup-lookingglass-v1` subprotocol. Reverse proxies must preserve
`Sec-WebSocket-Protocol` in both directions.
