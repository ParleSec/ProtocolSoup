...
- When the browser-based Looking Glass uses a separately hosted wallet, keep at least one wallet instance warm. A cold-start proxy failure occurs before the wallet can return the configured CORS headers.
