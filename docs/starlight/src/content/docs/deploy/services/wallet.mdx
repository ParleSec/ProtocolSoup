...
An agent that hits this origin should start at `GET /llms.txt` (or `Accept: text/markdown` on `/`). The process also serves `/.well-known/api-catalog`, `/.well-known/agent-skills/use-wallet-harness/SKILL.md`, and RFC 8288 `Link` headers on the HTML UI. This image is not an MCP server.
