...
**Fix:** upgrade `protocolsoup-wallet` to `v4.1.1` or later (the patched tag named in the GHSA). The wallet URL policy resolves DNS, requires HTTPS for untrusted hosts, re-validates redirects, dials only allowed addresses, and does not reflect upstream error bodies. See the [wallet service](/deploy/services/wallet/) runtime contract.
