...
ProtocolSoup is a Go backend with a React frontend. The backend uses a plugin system where each identity protocol is a self-contained module that registers its own HTTP routes, flow definitions, and demo scenarios. The frontend connects via a gateway that aggregates protocol metadata and proxies traffic to the correct backend service.
...
2. Frontend sends the request to the gateway via nginx proxy.
3. Gateway routes to the correct protocol service based on path prefix.
4. Protocol service executes the flow, generating real tokens, assertions, or credentials.
5. Each step emits a Looking Glass event over WebSocket.
6. Looking Glass renders every step, payload, and validation result in the browser in real time.
