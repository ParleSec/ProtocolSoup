...
Run live flows across 8 protocol families:

- **OAuth 2.0** -- Authorization Code, PKCE, Client Credentials, Refresh Token, Token Introspection, Token Revocation
...
- **SCIM 2.0** -- User Lifecycle, Group Membership, Filter Queries, Schema Discovery
- **SPIFFE/SPIRE** -- X.509-SVID Issuance, JWT-SVID Issuance, mTLS Handshake, Certificate Rotation
...
|---------|-------------|
| **Looking Glass** | Real-time execution console with step-by-step flow visualization, WebSocket event streaming, and SSF CAEP/RISC stream sessions |
| **Protocol Reference** | Guides, sequence diagrams, and security considerations for each shipped protocol family |
| **Wallet Harness** | Hosted OID4VCI/OID4VP wallet at [wallet.protocolsoup.com](https://wallet.protocolsoup.com) for issuance, presentation, QR handoff, and HAIP attestation. Agents: [wallet llms.txt](https://wallet.protocolsoup.com/llms.txt) |
...
4. For OID4VCI or OID4VP, open the [Wallet Harness](https://wallet.protocolsoup.com) to complete holder-side issuance or presentation.
5. Change one input and rerun to observe control behavior.
