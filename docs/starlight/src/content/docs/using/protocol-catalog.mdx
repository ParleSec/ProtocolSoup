...
|-------------|--------------------|
| Stream Lab | Default durable session (`/looking-glass?protocol=ssf`) |
...
**Demo scenarios:** SSF Stream Lab (Looking Glass), Push vs Poll Comparison

SSF stays a split service (`protocolsoup-ssf`). Looking Glass is the run surface: catalog rows are presets into one durable stream lab session.
