...
SSF is a **stream lab** in [Looking Glass](/using/looking-glass/): `/looking-glass?protocol=ssf`. Catalog flow IDs are presets into one durable Looking Glass session, not isolated grants. Fire CAEP/RISC events, inspect Transmitter and Receiver hops (full SET bodies), and read RP posture on the State tab.
...
| `caep-session-revoked` | Session Revoked (CAEP §3.1) | Session revocation event and response |
| `caep-credential-change` | Credential Change (CAEP §3.3) | Credential change event and response |
| `risc-account-disabled` | Account Disabled (RISC §2.3) | Account disabled event and response |
| `risc-credential-compromise` | Credential Compromise (RISC §2.7) | Credential compromise event and response |

## Demo Scenarios

- **SSF Stream Lab** -- Durable Looking Glass session for CAEP/RISC events
