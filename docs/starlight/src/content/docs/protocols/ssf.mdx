...
SSF is a **stream lab** in [Looking Glass](/using/looking-glass/): `/looking-glass?protocol=ssf`. Catalog flow IDs are presets into one durable Looking Glass session (they fill the event/delivery chrome). They do not replace the chrome.
...
|---------|------|--------------------|
| `ssf-stream-lab` | Stream Lab | Default lab session |
