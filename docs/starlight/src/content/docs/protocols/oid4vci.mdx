...
trust-anchor-gated API features. This is not a claim of complete HAIP issuer
support: DPoP sender-constrained access tokens are not implemented, and the
Looking Glass and wallet harness do not yet generate the client-attestation,
key-attestation, and encrypted-credential-request material for an end-to-end
HAIP issuance demo. The unattested Final flow remains unaffected by default.
