...
ProtocolSoup's OID4VCI Issuer and Wallet implementations are [OpenID Certified™](https://protocolsoup.com/trust#conformance) through the OpenID Foundation certification programme (HAIP, SD-JWT VC, and mdoc). Certification applies to the named versions and profiles, not every ProtocolSoup feature.
...
Every OID4VCI, HAIP, RFC 9449 and RFC 9901 requirement ProtocolSoup tracks has
its own page at [protocolsoup.com/spec/oid4vci](https://protocolsoup.com/spec/oid4vci)
(and `/spec/haip`, `/spec/rfc9449`, `/spec/rfc9901`). Each page quotes the
requirement, links the specification section, and shows ProtocolSoup's
self-test verdict for the build serving the page with links to the tests
behind it. These are self-test results, not OpenID Foundation certification;
see [Agent Discovery](/deploy/agent-discovery/#normative-requirement-pages) for
...
trust-anchor-gated (or otherwise explicitly enabled) API features. Named
certified ProtocolSoup versions and OID4VCI profiles are listed on the
[trust page](https://protocolsoup.com/trust#conformance); this section describes
the controls in the running demo, not a blanket claim for every deployment.
...
- **Key attestation (OID4VCI 1.0 Appendix D).** Credential configurations may
  ...
  `user_authentication` attack-potential levels. HAIP batch issuance from the
  hosted wallet puts every proof key in one Key Attestation JWT (HAIP 1.0
  Section 4.5.1); each jwt proof repeats that JWT in its `key_attestation` header.
...
- **Signed metadata and HAIP SD-JWT VC.** Requesting Credential Issuer
  ...
  List index served as a signed `statuslist+jwt`. The OID4VP verifier fetches
  that list during presentation and checks it with the same issuer key already
  accepted for the credential. The HAIP SD-JWT configuration's
