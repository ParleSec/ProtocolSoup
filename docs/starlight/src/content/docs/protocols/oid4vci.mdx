...
- **Pre-Authorized Issuance** -- Looking Glass creates a Credential Offer, then surfaces each real wallet→issuer hop (Credential Issuer Metadata, Token Request, Nonce Request, proof JWT, Credential Request/Response) from the wallet's protocol transcript. The default credential configuration is the ISO/IEC 18013-5 mobile driving licence (`mso_mdoc`).
...
- **Encrypted credential requests.** Credential Issuer Metadata advertises an
  ...
  `credential_request_encryption`. The Credential Endpoint accepts the compact
  JWE as `application/jwt`, decrypts it with the persistent key, and applies
  the same duplicate-member and proof validation as a JSON request.
