...
| `mso_mdoc` device key binding | Proof `cnf.jwk` must be an EC P-256 key; it is bound into the MSO `deviceKeyInfo.deviceKey` as a COSE_Key |
...
6. **Credential Response** — Issuer validates proof key binding, nonce freshness, and audience/subject constraints, then returns:
   - **Immediate**: `format` matching the credential configuration (e.g. `dc+sd-jwt` with disclosures, `jwt_vc_json`, `jwt_vc_json-ld`, `ldp_vc` with Data Integrity proof, or `mso_mdoc` as base64url-encoded CBOR `IssuerSigned`), `credential`, next `c_nonce`, or
...
- **Sender-constrained access tokens (RFC 9449 DPoP).** The token endpoint
  ...
  bound this way is then rejected at the credential, nonce, and
  deferred_credential endpoints unless presented as `Authorization: DPoP
...
- **Key attestation (OID4VCI 1.0 Appendix D).** Credential configurations may
  ...
  `cnf.jwk` holder key and satisfy any required `key_storage` /
...
- Proof JWT protected header for the default mDL `mso_mdoc`: `{"typ":"openid4vci-proof+jwt","alg":"ES256","kid":"..."}` with an EC P-256 `cnf.jwk` (the device key bound into the MSO). The SD-JWT VC and JOSE formats accept any wallet-supported signing algorithm.
