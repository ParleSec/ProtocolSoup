...
- **Pre-Authorized Issuance** -- Looking Glass creates a Credential Offer, then surfaces each real wallet→issuer hop (Credential Issuer Metadata, Token Request, Nonce Request, proof JWT, Credential Request/Response) from the wallet's protocol transcript. The default credential is the ISO/IEC 18013-5 mobile driving licence (`mso_mdoc`). Format and HAIP are independent selectors; HAIP shows an info callout that issuance uses wallet attestation, and the hosted wallet supplies that material. A 400 on import without attestation env is the real HAIP gate on a self-hosted wallet.
