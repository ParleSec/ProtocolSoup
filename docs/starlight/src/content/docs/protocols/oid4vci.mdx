...
- **Sender-constrained access tokens (RFC 9449 DPoP).** The token endpoint
  ...
  token (and, for the authorization_code grant, the refresh token) is bound
  to the proof's key via a `cnf.jkt` claim, with `token_type: DPoP` in the
  response instead of `Bearer`. A token bound this way is then rejected at
  the credential, nonce, and deferred_credential endpoints unless presented
  as `Authorization: DPoP <token>` with a matching, unreplayed DPoP proof —
  presenting it as a bare bearer token fails outright, never silently
  downgrades. A server-provided nonce challenge (`DPoP-Nonce` response
  header, `use_dpop_nonce` error) is available independently at the token
  endpoint and at the resource endpoints, off by default and enabled per
  role by deployment configuration. Only JWT access tokens can be bound;
  opaque/reference token binding is out of scope.
