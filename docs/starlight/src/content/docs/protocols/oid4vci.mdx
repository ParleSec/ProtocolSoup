...
| `oid4vci-issuer-initiated` | Issuer-Initiated Offer | 4 | Create an `authorization_code` Credential Offer with a fresh `issuer_state`, deliver it to an external Credential Offer Endpoint via a real HTTPS GET (or expose an `openid-credential-offer://` invocation/QR when no endpoint is set), then click Check Result to observe real issuer-observed PAR, token, and credential issuance milestones. Looking Glass does not redeem the code in-browser. |

## Demo Scenarios

- **Pre-Authorized Issuance** -- Looking Glass creates a Credential Offer, then surfaces each real wallet→issuer hop (Credential Issuer Metadata, Token Request, Nonce Request, proof JWT, Credential Request/Response) from the wallet's protocol transcript. The default credential configuration is the ISO/IEC 18013-5 mobile driving licence (`mso_mdoc`). HAIP key-attested profiles warn before Execute that the wallet needs attestation env material; a 400 without it is the real HAIP gate.
...
milestones via `status_uri`; it does not impersonate the wallet. Pointing that
flow's `credential_offer_endpoint` at the wallet (with HAIP attestation env
configured when the offer targets a HAIP configuration) is how a wallet-driven
authorization-code path is exercised.
