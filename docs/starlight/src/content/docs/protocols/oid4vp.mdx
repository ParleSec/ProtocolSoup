...
2. **Wallet Fetches Request Object** — Wallet fetches the compact signed JWT
   from `request_uri` via GET with media type
   `application/oauth-authz-req+jwt`. POST retrieval is explicitly unsupported
   and returns 405. The wallet validates signature, `typ`, and expiry.
