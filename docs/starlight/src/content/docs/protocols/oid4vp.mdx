...
ProtocolSoup's OID4VP Verifier and Wallet implementations are [OpenID Certified™](https://protocolsoup.com/trust#conformance) through the OpenID Foundation certification programme (HAIP, SD-JWT VC, and mdoc). Certification applies to the named versions and profiles, not every ProtocolSoup feature.
...
Every OID4VP, HAIP and RFC 9901 requirement ProtocolSoup tracks has its own
page at [protocolsoup.com/spec/oid4vp](https://protocolsoup.com/spec/oid4vp)
(and `/spec/haip`, `/spec/rfc9901`). Each page quotes the requirement, links
the specification section, and shows ProtocolSoup's self-test verdict for the
build serving the page with links to the tests behind it. These are self-test
results, not OpenID Foundation certification; see
[Agent Discovery](/deploy/agent-discovery/#normative-requirement-pages) for how
