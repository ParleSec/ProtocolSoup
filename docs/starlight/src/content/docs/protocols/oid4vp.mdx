...
The Looking Glass OID4VP controls preflight the selected VC credential profile against the DCQL query before wallet submission. The default preset and credential profile both target the mDL `mso_mdoc`. Looking Glass issuance profiles and DCQL presets are HAIP-aligned (`mso_mdoc` and `dc+sd-jwt`). HAIP key-attested issuance configurations are not offered on OID4VP: HAIP presentation is `x509_hash` plus encrypted `direct_post.jwt` (HAIP 1.0 Section 5), not wallet or key attestation (HAIP 1.0 Sections 4.4.1 and 4.5.1). Custom DCQL, including W3C `jwt_vc_json` / `jwt_vc_json-ld` / `ldp_vc`, can still be pasted; the wallet action is blocked unless the selected or pasted credential matches the requested format. Selecting `x509_hash` uses the HAIP signed-request profile: `response_mode` is `direct_post.jwt` and the query must be DCQL (scope alias is out of profile). On the unencrypted `direct_post` flow, Looking Glass shows that coercion before Execute. Wallet `x5c` trust failures against `WALLET_VERIFIER_X509_TRUST_ANCHOR_PEM` keep the protocol error; the hosted wallet already trusts the showcase verifier CA.
