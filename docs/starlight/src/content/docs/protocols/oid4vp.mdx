...
Browser-mediated W3C Digital Credentials API invocation and the HAIP wallet role
remain unsupported and are excluded from the current certification scope.
...
Digital Credentials API, and the wallet harness rejects these response modes.
These private APIs are implementation scaffolding, not a conforming DC API flow.
...
4. **Verifier Validates + Policy Decision** — Verifier requires exact `state`
   correlation for redirect response modes, then validates:
