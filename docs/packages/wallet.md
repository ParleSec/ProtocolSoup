...
- `GET /llms.txt`, `GET /llms-full.txt`, `GET /.well-known/api-catalog`, `GET /.well-known/agent-skills/use-wallet-harness/SKILL.md` — agent discovery (not protocol endpoints). Indexed from [docs.protocolsoup.com/llms.txt](https://docs.protocolsoup.com/llms.txt) and the repository README. Headless interface is the HTTP JSON API above; QR/deeplinks are human handoffs. Not an MCP server.
...
- **`credential configuration ... requires haip attestation material`:** this is the HAIP *issuance* gate on `POST /api/import` and OID4VP `/submit` auto-issue (HTTP 400). The hosted wallet is configured with `WALLET_CLIENT_ATTESTATION_ATTESTER_JWK_JSON` and `WALLET_CLIENT_ATTESTATION_KEY_ATTESTATION_JWK_JSON` (with `x5c`). A self-hosted wallet without those JWKs must set them or choose a non-HAIP configuration. OID4VP `/submit` issues the requested HAIP configuration (`MobileDrivingLicenceMsoMdocHAIP`, `UniversityDegreeCredentialSDJWTHAIP`); it does not rewrite to a general configuration.
