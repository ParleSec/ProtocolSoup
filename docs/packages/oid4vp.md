...
- **POST request URI**: create the request with `request_uri_method: "post"`.
  ...
  rejects a later fetch that changes it. Looking Glass exposes this as a
  "Request URI method" selector (GET/POST) alongside the verifier trust
  profile; selecting POST also appends `request_uri_method=post` to the
  generated `openid4vp://` wallet-handoff deep link.
