...
A standards-aligned implementation of OpenID for Verifiable Credential Issuance (OID4VCI) with real SD-JWT VC issuance, proof validation, `c_nonce` freshness enforcement, and deferred issuance support.
...
- **Real SD-JWT VC Output**: Issuer-signed VC payload plus selective-disclosure serialization
...
- Credential responses include SD-JWT style serialization (`typ=vc+sd-jwt`)
...
- SD-JWT VC profile for selective disclosure credential representation
