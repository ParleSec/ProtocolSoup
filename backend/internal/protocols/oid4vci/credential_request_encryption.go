...
func (p *Plugin) decodeCredentialRequest(r *http.Request, destination *credentialRequest) error {
	if requestHasMediaType(r, "application/json") {
		return decodeRequestObject(r.Body, destination, "credential request")
	}
	...
	if err != nil {
		return fmt.Errorf("credential request could not be read")
	}
	if len(raw) > maxEncryptedCredentialRequestBytes {
		return fmt.Errorf("credential request exceeds %d bytes", maxEncryptedCredentialRequestBytes)
	}
	...
	if err != nil {
		return fmt.Errorf("encrypted credential request is invalid: %w", err)
	}
	plaintext, err := encrypted.Decrypt(p.mdocPKI.DocumentSignerKey())
	if err != nil {
		return fmt.Errorf("encrypted credential request decryption failed")
	}
