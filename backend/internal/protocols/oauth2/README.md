...
| `implicit` | Implicit Grant (Legacy) | RFC 6749 §4.2 fragment access token; no refresh token. RFC 9700 §2.1.2 SHOULD NOT |
...
The authorization server advertises `response_types_supported: ["code", "token"]` and `grant_types_supported: ["authorization_code", "implicit", "refresh_token", "client_credentials", "urn:ietf:params:oauth:grant-type:device_code"]`, plus `device_authorization_endpoint`. Implicit (`response_type=token`) returns the access token in the redirect fragment and does not issue a refresh token. RFC 9700 §2.1.2 says clients SHOULD NOT use it; it is kept as a real educational contrast with authorization code + PKCE. `grant_type=implicit` is not accepted at the token endpoint. Resource Owner Password (`grant_type=password`) is not implemented; RFC 9700 §2.4 says the password grant MUST NOT be used. OIDC implicit remains a separate protocol flow.
...
|-----------|------|-------------|--------|
| `public-app` | Public | authorization_code, implicit, refresh_token, device_code | - |
...
- ✅ Implicit Grant (RFC 6749 §4.2; educational, RFC 9700 §2.1.2 SHOULD NOT)
